...
export const dealTransitionSchema = z.object({ dealId: z.string().min(1).max(160), toStage: z.enum(dealStages), offerAmount: z.number().positive().nullable(), actualAmount: z.number().positive().nullable(), commissionAmount: z.number().nonnegative().nullable(), currency: z.enum(currencyCodes).nullable(), lostReason: z.string().trim().min(2).max(500).nullable() }).strict().superRefine((value, context) => { if (value.toStage === "offer" && (value.offerAmount === null || value.currency === null)) context.addIssue({ code: "custom", message: "Teklif aşamasında tutar ve para birimi gerekli." }); if (value.toStage === "closed" && (value.actualAmount === null || value.commissionAmount === null || value.currency === null)) context.addIssue({ code: "custom", message: "Kapama aşamasında gerçekleşen bedel, komisyon ve para birimi gerekli." }); if (value.toStage === "lost" && !value.lostReason) context.addIssue({ code: "custom", message: "Kayıp nedeni gerekli." }); });
...
export function createDeal(draft: DealDraft, tenant: TenantOwned, now: number): Deal { const parsed = dealDraftSchema.parse(draft); return { ...tenant, ...parsed, stage: "presentation", offerAmount: null, actualAmount: null, commissionAmount: null, currency: null, lostReason: null, closedAt: null, deletedAt: null, createdAt: now, updatedAt: now }; }
