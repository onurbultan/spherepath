...
| Analytics periods | The funnel is computed live for 30-day, 90-day, and yearly windows from the records currently in Firestore. Stage durations, time to won or lost, rework, ask conversion, kept-follow-up rate, loss reasons, and closed commission by contact source are derived from stage events, interactions, and deals; results are withheld below a minimum sample. | No stored snapshots, no trend comparison between periods, and no office-level rollup. Every window is recomputed per request against the live collections and is bounded by their query limits, so a long window on a large workspace can under-report. Backfill and scheduled aggregation do not exist. |
...
| Contact views | Predefined segments, search, pagination, page selection, selected-contact export, and inline row actions are available. | User-defined saved views, shared office views, persisted column preferences, and server-side bulk jobs do not exist. |
| Deal closing | Presentation and deal stages are tracked. Closing a deal atomically marks its linked listing as sold or rented, and the realized amount and commission are totalled per currency on the funnel against the advisor's monthly portfolio target. | No commission rate rules or splits, deposit/payment schedule, contract document storage, e-signature provider, accounting integration, or settlement ledger exists. Closed deals missing an amount or currency are reported as incomplete rather than counted. |
| Search and matching | Structured buyer/tenant preferences are compared with office-pool inventory using deterministic, explainable metrics. Only transaction type, property type, and an explicit deal breaker disqualify; price, rooms, area, district, and missing must-haves lower the score and appear as near misses. A contact keeps up to three distinct property situations, each matched separately. | No map radius, travel-time, parcel/zoning registry, title-deed verification, live market valuation, or external inventory search exists. Extracted facts remain advisor-confirmed data. |
...
4. **Historical analytics:** persist daily office/advisor snapshots so longer windows stop depending on live query limits, then add period-over-period comparison and office rollups after backfill and data-quality checks.
