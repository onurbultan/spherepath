...
export function ConnectivityBanner() {
  ...
  useEffect(() => { if (!session) return; let active = true; void Promise.all(online ? [flushCaptureQueue(session), flushInboxQueue(session)] : [captureQueueCount(session.uid), inboxQueueCount(session.uid)]).then((counts) => { if (active) setQueued(counts.reduce((total, count) => total + count, 0)); }); return () => { active = false; }; }, [online, session]);
