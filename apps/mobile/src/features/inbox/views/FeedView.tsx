...
import { Archive, ArchiveRestore, Check, ChevronDown, ChevronUp, ListChecks, MapPin, Mic, Pencil, PhoneOff, Pin, Pyramid, RefreshCw, RotateCcw, Send, Settings, Shuffle, Sparkles } from "lucide-react-native";
import { type Href, router, useFocusEffect, useLocalSearchParams } from "expo-router";
import { useQuery, useQueryClient } from "@tanstack/react-query";
import { apiQueryKeys, commercialQueryKeys, dailyTaskResolutionLabels, inboxAnalysisHighlights, inboxItemTrace, inboxKindAfterAnalysis, isInboxItemResolved, type DailyTaskOutcome, type InboxItemKind, type InboxItemRecord, type TodayTask } from "@spherepath/shared";
...
import { applyNotePage, changeInboxItem, createInboxNote, flushInboxQueue, listInboxItems, retryInboxItem, undoInboxItem } from "../resources/inbox";
...
import { NotePageReview, pageSegmentCount } from "../components/NotePageReview";
...
export default function FeedView() {
  ...
  const inbox = useQuery({ queryKey: apiQueryKeys.inboxItems, enabled: Boolean(session), queryFn: () => listInboxItems(session ?? undefined), refetchInterval: (query) => (query.state.data as InboxItemRecord[] | undefined)?.some((item) => item.status === "queued" || item.status === "processing" || item.analysisStatus === "pending") ? 1_500 : false });
  const contacts = useQuery({ queryKey: apiQueryKeys.contacts, enabled: Boolean(session), queryFn: listContacts });
  const visibleNotes = (inbox.data ?? []).filter(inScope).map(interpretedItem);
  ...
  const creatingContactFor: string | null = null;
  function createContactFromNote(item: InboxItemRecord, _fullName: string, _phone: string | null) {
    setActiveNoteKind("person");
    setActiveNote(item);
  }
  async function updateItem(item: InboxItemRecord, update: { kind?: InboxItemKind; pinned?: boolean; archived?: boolean; analyze?: true }) {
    if (!session || item.id.startsWith("queued-")) return;
  ...
  // Keep the app usable while an additive Functions response rolls out.
  ...
  return <SafeAreaView edges={["top", "left", "right"]} style={[styles.safe, { backgroundColor: theme.background }]}>
    <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void Promise.all([today.refetch(), inbox.refetch()])} />} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">{contacts.data?.length === 0 ? <SpCard><SpText variant="title">{onboardingCopy.startTitle}</SpText><SpText color="secondary">{onboardingCopy.startHint}</SpText>{[[onboardingCopy.contact, "/(tabs)/contacts?create=1"], [onboardingCopy.capture, "/(tabs)/capture"], [onboardingCopy.listing, "/(tabs)/listings?action=add-listing"]].map(([label, path]) => <Pressable key={path} onPress={() => router.push(path as Href)}><SpText color="deed">{label}</SpText></Pressable>)}</SpCard> : null}
      {/* The tab bar holds five destinations; the funnel and settings are
          reached from here so neither is available on the web only. */}
      <View style={styles.header}><View><SpText variant="eyebrow" color="deed">{longDate.format(new Date()).toLocaleUpperCase("tr-TR")}</SpText><SpText variant="hero">Bugün</SpText></View><View style={styles.headerActions}><Pressable accessibilityLabel="Huni" onPress={() => router.push("/(tabs)/funnel")} style={[styles.iconButton, { borderColor: theme.line }]}><Pyramid size={18} color={theme.textSecondary} /></Pressable><Pressable accessibilityLabel="Ayarlar ve ekip" onPress={() => router.push("/(tabs)/settings")} style={[styles.iconButton, { borderColor: theme.line }]}><Settings size={18} color={theme.textSecondary} /></Pressable></View></View>
      ...
      <SpCard style={styles.dailyCard}>
        ...
        })}</View> : <View style={styles.empty}><SpText color="secondary">{upcomingTasks.length ? "Bugün için iş yok. Yaklaşan takiplerin aşağıda." : "Henüz planlanacak iş yok. İlk notunu veya kişini ekle."}</SpText></View>}
      ...
      {upcomingTasks.length ? <SpCard style={styles.dailyCard}><View style={styles.sectionTitle}><View><SpText variant="eyebrow" color="deed">YAKLAŞAN</SpText><SpText variant="title">Yarın ve sonrası</SpText></View><SpText variant="bodySmall" color="secondary">{upcomingTasks.length} iş</SpText></View><View style={styles.taskList}>{upcomingTasks.slice(0, 8).map((task) => <Pressable key={task.id} accessibilityLabel={`${task.title} kaydını aç`} onPress={() => router.push(taskRecordRoute(task) as never)} style={[styles.additionalTask, { borderTopColor: theme.line }]}><SpText>{task.title}</SpText><SpText variant="bodySmall" color="secondary">{task.reason} · {taskDueLabel(task.dueAt)}</SpText></Pressable>)}</View></SpCard> : null}

      {recentInteractions.length ? <SpCard style={styles.dailyCard}><View style={styles.sectionTitle}><View><SpText variant="eyebrow" color="deed">GÜNÜN HAFIZASI</SpText><SpText variant="title">Bugün kaydedilen temaslar</SpText></View><SpText variant="bodySmall" color="secondary">{recentInteractions.length} temas</SpText></View><View style={styles.taskList}>{recentInteractions.map((interaction) => <Pressable key={interaction.id} accessibilityLabel={`${interaction.contactName} kaydını aç`} onPress={() => router.push(`/contact/${interaction.contactId}` as never)} style={[styles.additionalTask, { borderTopColor: theme.line }]}><SpText>{interaction.contactName}</SpText><SpText variant="bodySmall" color="secondary">{interaction.outcome}</SpText><SpText variant="caption" color="secondary">{new Intl.DateTimeFormat("tr-TR", { hour: "2-digit", minute: "2-digit" }).format(interaction.occurredAt)}</SpText></Pressable>)}</View></SpCard> : null}
    ...
    /> : null}
    {currentActiveNote ? <NoteProcessingSheet key={currentActiveNote.id} item={currentActiveNote} contacts={contacts.data ?? []} initialKind={activeNoteKind} onClose={() => { setActiveNote(null); setActiveNoteKind(undefined); }} onChanged={async (updatedItem) => {
      ...
      await Promise.all([client.invalidateQueries({ queryKey: apiQueryKeys.inboxItems }), client.invalidateQueries({ queryKey: apiQueryKeys.contacts }), client.invalidateQueries({ queryKey: apiQueryKeys.opportunities }), client.invalidateQueries({ queryKey: apiQueryKeys.portfolioItems }), client.invalidateQueries({ queryKey: apiQueryKeys.todayOverview })]);
    }} /> : null}
  </SafeAreaView>;
}
