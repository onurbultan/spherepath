...
export default function FeedView() {
  ...
  return <SafeAreaView edges={["top", "left", "right"]} style={[styles.safe, { backgroundColor: theme.background }]}>
    <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => void Promise.all([today.refetch(), inbox.refetch()])} />} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.noteHeading}><View style={styles.flex}><SpText variant="title">Notların</SpText><SpText variant="bodySmall" color="secondary">Sistem tür önerir; gerçek kayda dönüştürmeye sen karar verirsin.</SpText></View><View style={[styles.noteToggle, { borderColor: theme.line }]}><Pressable onPress={() => setShowArchived(false)} style={[styles.toggleButton, !showArchived && { backgroundColor: theme.deedBg }]}><SpText variant="bodySmall" color={!showArchived ? "deed" : "secondary"}>Aktif</SpText></Pressable><Pressable onPress={() => setShowArchived(true)} style={[styles.toggleButton, showArchived && { backgroundColor: theme.deedBg }]}><SpText variant="bodySmall" color={showArchived ? "deed" : "secondary"}>Arşiv</SpText></Pressable></View></View>
      {inbox.isPending ? <ActivityIndicator color={theme.deed} /> : inbox.data?.some((item) => (item.status === "archived") === showArchived) ? <View style={styles.grid}>{inbox.data.filter((item) => (item.status === "archived") === showArchived).map((item) => {
...
const styles = StyleSheet.create({
  safe: { flex: 1 }, flex: { flex: 1 }, content: { padding: space.lg, paddingBottom: 120, gap: space.lg }, header: { flexDirection: "row", alignItems: "center", justifyContent: "space-between" }, iconButton: { width: 44, height: 44, borderWidth: StyleSheet.hairlineWidth, borderRadius: radius.md, alignItems: "center", justifyContent: "center" }, dailyCard: { gap: space.lg }, sectionTitle: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: space.md }, taskList: { gap: 0 }, task: { minHeight: 64, borderTopWidth: StyleSheet.hairlineWidth, flexDirection: "row", alignItems: "center", gap: space.md, paddingVertical: space.sm }, taskNumber: { width: 30, height: 30, borderRadius: 15, alignItems: "center", justifyContent: "center" }, taskCopy: { flex: 1 }, doneText: { textDecorationLine: "line-through", opacity: .55 }, smallAction: { width: 44, height: 44, alignItems: "center", justifyContent: "center" }, retryAction: { minHeight: 44, paddingHorizontal: space.sm, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 4 }, editAction: { minHeight: 44, marginRight: "auto", paddingHorizontal: space.sm, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 5 }, complete: { width: 44, height: 44, borderRadius: 22, alignItems: "center", justifyContent: "center" }, empty: { paddingVertical: space.md }, composer: { gap: space.lg }, sharedSource: { padding: space.sm, borderRadius: radius.sm }, input: { minHeight: 108, borderWidth: StyleSheet.hairlineWidth, borderRadius: radius.md, padding: space.lg, fontFamily: "Karla_400Regular", fontSize: 16, lineHeight: 23, textAlignVertical: "top" }, composerActions: { flexDirection: "row", gap: space.sm, justifyContent: "flex-end" }, secondary: { minHeight: 46, borderWidth: StyleSheet.hairlineWidth, borderRadius: radius.md, paddingHorizontal: space.lg, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: space.sm }, primary: { minHeight: 46, borderRadius: radius.md, paddingHorizontal: space.xl, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: space.sm }, alert: { borderRadius: radius.md, padding: space.md }, feedHeading: { marginTop: space.md, gap: 2 }, noteHeading: { marginTop: space.md, flexDirection: "row", alignItems: "flex-end", gap: space.md }, noteToggle: { flexDirection: "row", padding: 3, borderWidth: StyleSheet.hairlineWidth, borderRadius: radius.md }, toggleButton: { minHeight: 36, paddingHorizontal: space.sm, alignItems: "center", justifyContent: "center", borderRadius: radius.sm }, grid: { flexDirection: "row", flexWrap: "wrap", gap: space.md }, noteCard: { width: "48%", minWidth: 150, flexGrow: 1, gap: space.md, borderWidth: 1 }, noteMeta: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: space.sm }, kind: { paddingHorizontal: space.sm, paddingVertical: 5, borderRadius: radius.sm }, prompt: { flexDirection: "row", alignItems: "center", gap: space.sm, padding: space.sm, borderRadius: radius.sm }, noteActions: { flexDirection: "row", flexWrap: "wrap", alignItems: "center", justifyContent: "flex-end", marginTop: "auto" }, expandAction: { minHeight: 44, flexDirection: "row", alignItems: "center", gap: 4 }, locationForm: { flexDirection: "row", alignItems: "center", gap: space.sm }, locationInput: { flex: 1, minHeight: 44, borderWidth: StyleSheet.hairlineWidth, borderRadius: radius.sm, paddingHorizontal: space.md, fontFamily: "Karla_400Regular", fontSize: 14 }, locationSubmit: { minHeight: 44, borderRadius: radius.sm, paddingHorizontal: space.lg, alignItems: "center", justifyContent: "center" },
});
