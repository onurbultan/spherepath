...
import { apiQueryKeys, dailyTaskResolutionLabels, inboxAnalysisHighlights, inboxItemTrace, isInboxItemResolved, joinPhone, splitPhone, type DailyTaskOutcome, type InboxItemKind, type InboxItemRecord, type TodayTask } from "@spherepath/shared";
...
import { changeInboxItem, createInboxNote, flushInboxQueue, listInboxItems, retryInboxItem, undoInboxItem, processInboxItem } from "../resources/inbox";
...
export default function FeedView() {
  ...
  // The note names someone the workspace has never seen; making the advisor pick
  // a type and retype that name is the system asking for what it just read.
  async function createContactFromNote(item: InboxItemRecord, fullName: string, phone: string | null) {
    if (!session) return;
    setCreatingContactFor(item.id);
    try {
  ...
  return <SafeAreaView edges={["top", "left", "right"]} style={[styles.safe, { backgroundColor: theme.background }]}>
    ...
    {activeNote ? <NoteProcessingSheet key={activeNote.id} item={activeNote} contacts={contacts.data ?? []} onClose={() => setActiveNote(null)} onChanged={async (updatedItem) => {
