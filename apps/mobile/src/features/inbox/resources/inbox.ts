...
export async function createInboxNote(session: WorkspaceSession, text: string, source: "typed" | "whatsapp" = "typed", dayKey: string | null = null): Promise<InboxItemRecord> {
  const input: CreateInboxItemInput = { source, text, linkedContactId: null, requestedKind: null, dayKey };
