...
export function NoteProcessingSheet({ item, contacts, initialKind, onClose, onChanged }: { item: InboxItemRecord; contacts: readonly ContactRecord[]; initialKind?: InboxItemKind; onClose(): void; onChanged(updatedItem?: InboxItemRecord): Promise<void> | void }) {
  ...
  const [personName, setPersonName] = useState(item.analysis?.insights.contactName?.trim() || inferred.explicitContact?.fullName || inboxContactName(item.safeText) || ""); const [personPhone, setPersonPhone] = useState(item.analysis?.insights.contactPhone?.trim() || inboxContactPhone(item.safeText) || "");
  ...
  async function process() {
    ...
    try {
      ...
      if (creatingPerson) processedItem = (await processItem(session, { inboxItemId: activeItem.id, action: "person", contact: contactDraftSchema.parse({ fullName: personName, phone: personPhone, metAtPlace, source: personSource, role: kind === "requirement" ? roleForOpportunity(opportunityType) : personRole, nextActionType: actionType, nextActionAt }), ...(analysis || item.analysis ? { approvedInsights: analysis ? reviewedInboxInsights(analysis.insights, opportunityType) : item.analysis!.insights } : {}), recordInteraction: true, opportunityType: kind === "requirement" || createPersonOpportunity ? opportunityType : null })).item;
      else if (kind === "requirement") { if (!contactId) throw new Error("Talebi oluşturmak için kişiyi seç."); if (!analysis) throw new Error("Önce talep bilgilerini çıkarıp kontrol et."); processedItem = (await processItem(session, { inboxItemId: activeItem.id, action: "requirement", contactId, opportunityType: requirementOpportunity(opportunityType), nextActionType: actionType, nextActionAt, approvedInsights: reviewedInboxInsights(analysis.insights, opportunityType) })).item; }
