...
export function NoteProcessingSheet({ item, contacts, onClose, onChanged }: { item: InboxItemRecord; contacts: readonly ContactRecord[]; onClose(): void; onChanged(): Promise<void> | void }) {
  ...
  const [actionType, setActionType] = useState<NextActionType>("call"); const [actionDays, setActionDays] = useState(1);
  ...
  async function process() {
    ...
    try {
      await saveEdits(); const nextActionAt = Date.now() + actionDays * 86_400_000;
      ...
      else if (kind === "requirement") { if (!contactId) throw new Error("Talebi oluşturmak için kişiyi seç."); await processItem(session, { inboxItemId: activeItem.id, action: "requirement", contactId, opportunityType, nextActionType: actionType, nextActionAt }); }
