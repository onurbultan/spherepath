...
export function OpportunityAction({ opportunity, compact = false }: { opportunity: OpportunityRecord; compact?: boolean }) {
  ...
  return <><SpButton label={compact ? advisorWorkflowCopy.editActionShort : advisorWorkflowCopy.editAction} onPress={() => setOpen(true)} />{open ? <TaskResolutionSheet initialStatus="rescheduled" task={task} pending={pending} error={error} onClose={() => setOpen(false)} onResolve={(outcome) => void save(outcome)} /> : null}</>;
}
