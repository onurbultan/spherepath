...
export interface PortfolioMatchResult {
  matches: PortfolioMatchRecord[];
  nearMisses: PortfolioMatchRecord[];
}
...
export async function analyzePortfolioText(text: string, source: PortfolioSource = "manual"): Promise<PortfolioItemDraft> {
  return (await apiClient.query<{ text: string; source: PortfolioSource }, { draft: PortfolioItemDraft }>(
    "extractPortfolioText", { text, source },
  )).draft;
}

export async function savePortfolioItem(session: WorkspaceSession, draft: PortfolioItemDraft): Promise<PortfolioItemRecord> {
  return (await apiClient.command<PortfolioItemDraft, { portfolioItem: PortfolioItemRecord }>(
    "createPortfolioItemFromDraft", draft, createCommandId(session.uid),
  )).portfolioItem;
}

export async function withdrawPortfolioItem(session: WorkspaceSession, portfolioItemId: string): Promise<void> {
  await apiClient.command<{ portfolioItemId: string }, { portfolioItemId: string }>(
    "withdrawPortfolioItem", { portfolioItemId }, createCommandId(session.uid),
  );
}
