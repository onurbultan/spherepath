import { SpField, SpInput , choiceMetrics, controlMetrics, largeButtonMetrics } from "@/shared/ui/SpField";
import { getClosingOverview } from "@/features/closing/resources/closing";
import { advisorWorkflowCopy, parseMoneyInput, type CurrencyCode ,
  apiQueryKeys,
  askOutcomeLabels,
  askOutcomes,
  contactDraftSchema,
...
export default function CaptureView() {
  ...
  useEffect(() => {
    void AsyncStorage.getItem("spherepath.captureMode").then((stored) => {
  ...
  function changeCaptureMode(mode: "voice" | "manual") {
    setCaptureMode(mode);
    void AsyncStorage.setItem("spherepath.captureMode", mode);
  }
  ...
  useEffect(() => {
    ...
    const date = new Date(selectedContact.relationship.nextActionAt);
    setNextActionAtValue(new Date(date.getTime() - date.getTimezoneOffset() * 60_000).toISOString().slice(0, 16));
  ...
  async function createQuickContact() {
    ...
    try {
      ...
      setContactId(contact.id); clearFollowUpContext();
  ...
  async function submit() {
    ...
    const parsed = manualInteractionSchema.safeParse({
      ...
      noteSummary,
      occurredAt: occurredAtValue ? new Date(occurredAtValue).getTime() : hoursAgo === null ? null : Date.now() - hoursAgo * 60 * 60 * 1_000,
    });
    if (!parsed.success) {
  ...
  function reset() {
    ...
    setOccurredAtValue(""); setHoursAgo(null);
    setContactId("");
  ...
  return (
    <SafeAreaView edges={["top", "left", "right"]} style={[styles.safe, { backgroundColor: theme.background }]}>
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <View style={styles.header}><SpText variant="eyebrow" color="ask">HIZLI KAYIT</SpText><SpText variant="hero">Temas kaydet</SpText><SpText color="secondary">Görüşme sonucunu ve planlanan sonraki adımı kısa biçimde kapat.</SpText></View>
