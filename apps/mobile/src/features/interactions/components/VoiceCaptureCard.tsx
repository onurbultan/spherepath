...
import { AlertTriangle, Check, LoaderCircle, Mic, Square, Trash2 } from "lucide-react-native";
import {
  RecordingPresets,
...
import { confirmVoiceNote, discardVoiceNote, getLatestReviewableVoiceNote, getVoiceNote, uploadAndRegisterVoiceNote } from "../resources/interactions";
...
function formatRoomPreference(insights: VoiceInsights): string | null {
  const item = insights.propertyPreferences;
  if (item.bedroomCountMin !== null) return `En az ${item.bedroomCountMin}${item.livingRoomCountMin !== null ? `+${item.livingRoomCountMin}` : ""}`;
...
export function VoiceCaptureCard({ session, contacts, onSaved }: Props) {
  ...
  const applyReview = useCallback((note: VoiceNoteView) => {
    ...
    // Mobile keeps the existing explicit opportunity flow until its compact
    // review control is added; web exposes the one-confirmation option now.
  ...
  async function submitReview() {
    ...
    const parsed = manualInteractionSchema.safeParse({
      ...
      askOutcome,
      nextActionType,
      nextActionAt: nextActionType && nextActionDays !== null ? nextActionTimestamp(nextActionDays, nextActionTime) : null,
      noteSummary,
    });
    if (!parsed.success) {
...
function VoiceInsightsReview({ insights, preferenceLabels, approvedKeyThings, includePropertyPreferences, onKeyThingsChange, onIncludePropertyPreferencesChange }: { insights: VoiceInsights | null | undefined; preferenceLabels: string[]; approvedKeyThings: string[]; includePropertyPreferences: boolean; onKeyThingsChange(value: string[]): void; onIncludePropertyPreferencesChange(value: boolean): void }) {
  const theme = useSpTheme();
  if (!insights || (!insights.keyThingsToRemember.length && !preferenceLabels.length && !insights.suggestedActionReason)) return null;
  return <View style={styles.insights}>{insights.keyThingsToRemember.length ? <View style={styles.stack}><SpText variant="eyebrow" color="deed">HATIRLANACAKLAR</SpText>{insights.keyThingsToRemember.map((item) => { const selected = approvedKeyThings.includes(item); return <Pressable key={item} onPress={() => onKeyThingsChange(selected ? approvedKeyThings.filter((value) => value !== item) : [...approvedKeyThings, item])} style={[styles.memoryItem, { borderColor: selected ? theme.deed : theme.line, backgroundColor: theme.background }]}><View style={[styles.checkbox, { borderColor: selected ? theme.deed : theme.line, backgroundColor: selected ? theme.deed : "transparent" }]}>{selected ? <Check color={theme.onDeed} size={13} /> : null}</View><SpText variant="bodySmall" style={styles.flex}>{item}</SpText></Pressable>; })}</View> : null}{preferenceLabels.length ? <View style={styles.stack}>{insights.propertyContext === "subject_property" ? <View style={styles.insightToggle}><View style={styles.flex}><SpText variant="bodySmall">Görüşülen gayrimenkul</SpText><SpText variant="caption" color="secondary">Bu bilgiler kişinin arama tercihlerine eklenmez.</SpText></View></View> : <Pressable onPress={() => onIncludePropertyPreferencesChange(!includePropertyPreferences)} style={styles.insightToggle}><View style={[styles.checkbox, { borderColor: includePropertyPreferences ? theme.deed : theme.line, backgroundColor: includePropertyPreferences ? theme.deed : "transparent" }]}>{includePropertyPreferences ? <Check color={theme.onDeed} size={13} /> : null}</View><View style={styles.flex}><SpText variant="bodySmall">Gayrimenkul tercihlerini kişi hafızasına ekle</SpText><SpText variant="caption" color="secondary">Yanlış bir çıkarım varsa bu seçimi kaldırabilirsiniz.</SpText></View></Pressable>}<View style={styles.choices}>{preferenceLabels.map((item) => <View key={item} style={[styles.insightChip, { backgroundColor: theme.deedBg }]}><SpText variant="caption" color="deed">{item}</SpText></View>)}</View></View> : null}{insights.suggestedActionReason ? <View style={[styles.actionReason, { backgroundColor: theme.askBg }]}><SpText variant="bodySmall" color="ask">Önerilen aksiyon</SpText><SpText variant="caption" color="secondary">{insights.suggestedActionReason}</SpText></View> : null}</View>;
}

const styles = StyleSheet.create({
  card: { gap: space.lg }, heading: { flexDirection: "row", alignItems: "center", gap: space.md }, icon: { width: 42, height: 42, borderRadius: 21, alignItems: "center", justifyContent: "center" }, flex: { flex: 1 }, stack: { gap: space.md }, choices: { flexDirection: "row", flexWrap: "wrap", gap: space.sm }, choice: { minHeight: 38, borderRadius: radius.sm, borderWidth: StyleSheet.hairlineWidth, justifyContent: "center", paddingHorizontal: space.md }, confirm: { minHeight: 68, borderRadius: radius.md, borderWidth: StyleSheet.hairlineWidth, flexDirection: "row", alignItems: "center", gap: space.md, padding: space.md }, checkbox: { width: 22, height: 22, borderRadius: 5, borderWidth: 1, alignItems: "center", justifyContent: "center" }, primary: { minHeight: 50, borderRadius: radius.md, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: space.sm }, secondary: { minHeight: 50, borderRadius: radius.md, borderWidth: StyleSheet.hairlineWidth, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: space.sm, paddingHorizontal: space.md }, reviewActions: { flexDirection: "row", alignItems: "center", gap: space.sm, flexWrap: "wrap" }, recording: { minHeight: 72, borderRadius: radius.md, flexDirection: "row", alignItems: "center", gap: space.md, padding: space.md }, dot: { width: 12, height: 12, borderRadius: 6 }, stop: { minHeight: 40, borderRadius: radius.sm, borderWidth: StyleSheet.hairlineWidth, flexDirection: "row", alignItems: "center", gap: space.sm, paddingHorizontal: space.md }, processing: { minHeight: 68, borderRadius: radius.md, flexDirection: "row", alignItems: "center", gap: space.md, padding: space.md }, warning: { borderRadius: radius.sm, flexDirection: "row", alignItems: "center", gap: space.sm, padding: space.md }, input: { minHeight: 48, borderRadius: radius.md, borderWidth: StyleSheet.hairlineWidth, padding: space.md, fontFamily: "Karla_400Regular", fontSize: 16 }, multiline: { minHeight: 84, textAlignVertical: "top" }, insights: { gap: space.md, padding: space.md, borderRadius: radius.md }, memoryItem: { minHeight: 48, borderRadius: radius.sm, borderWidth: StyleSheet.hairlineWidth, flexDirection: "row", alignItems: "center", gap: space.sm, padding: space.md }, insightToggle: { flexDirection: "row", alignItems: "flex-start", gap: space.sm }, insightChip: { paddingHorizontal: space.md, paddingVertical: space.sm, borderRadius: radius.sm }, actionReason: { gap: space.xs, padding: space.md, borderRadius: radius.sm },
});
