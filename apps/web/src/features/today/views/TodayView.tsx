    ...
    <AppShell>
      <div className="today-layout">
        ...
        {query.isPending && !overview ? <div className="content-state"><RefreshCw className="spin" size={22} aria-hidden /> Bugün görünümü hazırlanıyor…</div> : overview ? <>
          ...
          {fullOverview?.stages.acquaintance === 0 ? <SpCard className="onboarding-card"><div><p className="eyebrow">HEMEN BAŞLA</p><h2>Şu an hangi işi yapmak istiyorsun?</h2><p className="context-sentence">Ayarlar daha sonra tamamlanabilir. İlk gerçek kaydı bulunduğun işten oluştur.</p></div><ol><li><Link href="/note"><strong>Günün notunu yaz</strong><span>Aklındakini olduğu gibi yaz; satırları kişiye, portföye ve işe sonra birlikte dönüştürün</span></Link></li><li><Link href="/contacts?next=listings"><strong>Mevcut portföy ekle</strong><span>Önce mülk sahibini ekle, ardından yetki bilgilerine devam et</span></Link></li><li><Link href="/contacts"><strong>Kişi ağını oluştur</strong><span>Rehberinden veya elle ilk kişiyi ekle</span></Link></li></ol></SpCard> : null}
