    ...
    <AppShell>
      ...
      {captureMode === "voice" ? <VoiceCaptureCard key={selectedContactId} session={session!} contacts={contacts} initialContactId={selectedContactId} onSaved={async () => {
        ...
        <form className="capture-form" onSubmit={submit}>
          <SpCard className="form-section"><div className="section-heading compact"><div><p className="eyebrow">1 · KİM</p><h2>Görüşülen kişi</h2></div></div><div className="form-row"><ContactCombobox contacts={contacts} value={selectedContactId} onChange={setContactId} /><label>Kanal<SpSelect value={channel} onChange={(event) => setChannel(event.target.value as ManualInteractionDraft["channel"])}>{interactionChannels.map((item) => <option key={item} value={item}>{interactionChannelLabels[item]}</option>)}</SpSelect></label></div></SpCard>
          ...
          <SpCard className="form-section"><p className="eyebrow">3 · SONRAKİ ADIM</p><h2>Takibi planla</h2><div className="form-row"><label>Aksiyon<SpSelect value={nextActionType ?? ""} onChange={(event) => { const value = (event.target.value || null) as ManualInteractionDraft["nextActionType"]; setNextActionType(value); if (!value) setNextActionAt(""); }}><option value="">Henüz yok</option>{nextActionTypes.map((item) => <option key={item} value={item}>{nextActionTypeLabels[item]}</option>)}</SpSelect></label><QuickDateField disabled={!nextActionType} label="Tarih · saat isteğe göre değiştirilebilir" required={Boolean(nextActionType)} value={nextActionAt} onChange={setNextActionAt} /></div><SpInput name="nextActionAt" type="hidden" value={nextActionAt} /></SpCard>
