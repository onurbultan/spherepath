...
export function SettingsView() {
  ...
  return <AppShell>
    <header className="page-header settings-header"><div><p className="eyebrow">ÇALIŞMA ALANI</p><h1>Ayarlar ve uyum</h1><p className="context-sentence">Profil, ofis yükümlülükleri, hatırlatmalar ve veri sahibi talepleri.</p></div><div className="header-actions">{editedDraft ? <span className="unsaved-label">Kaydedilmemiş değişiklikler</span> : null}<button className="secondary-action inline-action" disabled={!editedDraft || pending} type="button" onClick={() => setDraft(null)}>Vazgeç</button><button className="primary-action inline-action" disabled={pending} form="workspace-settings-form" type="submit"><Save size={16} /> {pending ? "Kaydediliyor…" : "Kaydet"}</button></div></header>
    {message ? <p className="success-notice">{message}</p> : null}{error ? <p className="form-error notice">{error}</p> : null}
    <div className="settings-workspace">
      <nav className="settings-subnav" aria-label="Ayar bölümleri"><div><span>Hesap</span><a href="#advisor-profile"><UserRoundCog size={16} /> Danışman profili</a><a href="#advisor-profile"><Bell size={16} /> Hatırlatmalar</a></div><div><span>Ofis</span><a href="#office-team"><Users size={16} /> Ekip ve davetler <em>{teamQuery.data?.members.length ?? 0}</em></a><a className="active" href="#data-controller"><ShieldCheck size={16} /> Veri sorumlusu</a></div><div><span>Uyum</span><a href="#data-requests"><FileText size={16} /> Veri sahibi talepleri <em>{pendingRequests.length}</em></a><button disabled title="Ses kayıt politikaları henüz ayrı bir ayar ekranına sahip değil" type="button"><Lock size={16} /> Ses ve gizlilik</button><button disabled title="Toplu çalışma alanı dışa aktarımı henüz desteklenmiyor" type="button"><Download size={16} /> Veri dışa aktarma</button></div></nav>
    ...
    <span className="anchor-target" id="advisor-profile" />
    <span className="anchor-target" id="data-controller" />
    <form className="settings-grid" id="workspace-settings-form" onSubmit={save}>
      <SpCard className="settings-card"><div className="settings-title"><UserRoundCog size={20} /><div><p className="eyebrow">PROFİL</p><h2>Danışman ayarları</h2></div></div><label>Ad soyad<input value={draft.displayName} onChange={(event) => setDraft({ ...draft, displayName: event.target.value })} /></label><label>Telefon<input value={draft.phone} onChange={(event) => setDraft({ ...draft, phone: event.target.value })} /></label><label>Varsayılan bölgeler <span className="optional">virgülle ayır</span><input value={draft.defaultRegions.join(", ")} onChange={(event) => setDraft({ ...draft, defaultRegions: event.target.value.split(",").map((item) => item.trim()).filter(Boolean).slice(0, 5) })} /></label><div className="form-row"><label>Aylık portföy hedefi<input type="number" min="1" max="100" value={draft.monthlyPortfolioTarget ?? ""} onChange={(event) => setDraft({ ...draft, monthlyPortfolioTarget: event.target.value ? Number(event.target.value) : null })} /></label><label>Haftalık kapasite<input type="number" min="1" max="100" value={draft.weeklyCapacity ?? ""} onChange={(event) => setDraft({ ...draft, weeklyCapacity: event.target.value ? Number(event.target.value) : null })} /></label></div><label className="check-label"><input type="checkbox" checked={draft.dailyPlanReminderEnabled} onChange={(event) => setDraft({ ...draft, dailyPlanReminderEnabled: event.target.checked })} /> Günlük plan hatırlatıcısı</label><div className="form-row"><label>Saat<input type="number" min="0" max="23" disabled={!draft.dailyPlanReminderEnabled} value={draft.dailyPlanReminderHour} onChange={(event) => setDraft({ ...draft, dailyPlanReminderHour: Number(event.target.value) })} /></label><label>Dakika<input type="number" min="0" max="59" disabled={!draft.dailyPlanReminderEnabled} value={draft.dailyPlanReminderMinute} onChange={(event) => setDraft({ ...draft, dailyPlanReminderMinute: Number(event.target.value) })} /></label></div></SpCard>
    ...
    <span className="anchor-target" id="office-team" />
    ...
    <span className="anchor-target" id="data-requests" />
