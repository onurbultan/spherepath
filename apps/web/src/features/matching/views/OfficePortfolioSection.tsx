...
const matchMessageSubject = (match: PortfolioMatchRecord) => ({
  ...
  mismatchReasons: match.reasons.filter((reason) => reason.status === "mismatch").map((reason) => reason.detail),
...
export function PortfolioMatchCard({ match, nearMiss = false }: { match: PortfolioMatchRecord; nearMiss?: boolean }) {
  ...
  async function prepareMessage() {
    ...
    try {
      draft = await draftMatchMessage({ contactId: match.contactId, portfolioItemId: match.portfolioItem.id, opportunityId: match.opportunityId ?? null });
    } catch {
      // keep the template
    }
  ...
  const shownReasons = [...match.reasons.filter((reason) => reason.status === "mismatch"), ...match.reasons.filter((reason) => reason.status !== "mismatch")];
  ...
  return <SpCard className={nearMiss ? "match-card is-near-miss" : "match-card"}>
    ...
    {nearMiss ? <p>{match.softMismatchKeys.length ? advisorWorkflowCopy.alternativeMatch : advisorWorkflowCopy.incompleteMatch}</p> : null}
    {match.situationSummary ? <p className="match-situation">Bu talebi için: {match.situationSummary}</p> : null}
    <ul>{shownReasons.map((reason) => <li className={`match-reason-${reason.status}`} key={reason.key}>{reason.status === "unknown" ? "Doğrulanmalı: " : ""}{reason.detail} ({reason.status === "match" ? reason.weight : 0}/{reason.weight} puan)</li>)}</ul>
    ...
    {copyState === "copied" && usedTemplate ? <p className="compact-error">İncelediğin şablon mesaj kopyalandı.</p> : null}
...
export function OfficePortfolioSection({ openSignal = 0 }: { openSignal?: number }) {
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      await Promise.all([queryClient.invalidateQueries({ queryKey: apiQueryKeys.portfolioItems }), queryClient.invalidateQueries({ queryKey: apiQueryKeys.portfolioMatches }), queryClient.invalidateQueries({ queryKey: apiQueryKeys.matchNotifications })]);
  ...
  return <section className="office-pool-section" id="office-pool" aria-labelledby="office-pool-title">
    <label>Talep sahibi<SpSelect value={contactFilter} onChange={(event) => { setContactFilter(event.target.value); setVisibleCount(12); }}><option value="">Tüm talepler</option>{[...new Map([...(matchesQuery.data?.matches ?? []), ...(matchesQuery.data?.nearMisses ?? [])].map((item) => [item.contactId, item.contactName])).entries()].map(([id, name]) => <option key={id} value={id}>{name}</option>)}</SpSelect></label>
    <div className="inline-section-heading"><div><h2 id="office-pool-title">Talep ve portföy eşleşmeleri</h2><span>satılık–alıcı ve kiralık–kiracı taleplerinin açıklanabilir kesişimi</span></div><div className="office-pool-heading-actions"><button className="text-button" disabled={!items.length} onClick={() => setShowPool((current) => !current)} type="button">{showPool ? "Eşleşmelere dön" : `Tüm havuzu gör · ${items.length}`}</button></div></div>
    {!showPool && matches.length ? <div className="match-strip" aria-label="Uygun eşleşmeler">{matches.slice(0, visibleCount).map((match) => <PortfolioMatchCard key={`${match.opportunityId ?? match.contactId}-${match.portfolioItem.id}`} match={match} />)}</div> : null}
    {!showPool && nearMisses.length ? <div className="near-miss-block"><div className="near-miss-heading"><p className="eyebrow">BİLGİ EKSİK VEYA ALTERNATİF</p><p className="context-sentence">Bilgi kapsamını ve farklı kriterleri incele. Eksikleri inceleyip müşteriyle doğrula.</p></div><div className="match-strip" aria-label="Yakın eşleşmeler">{nearMisses.slice(0, visibleCount).map((match) => <PortfolioMatchCard key={`near-${match.opportunityId ?? match.contactId}-${match.portfolioItem.id}`} match={match} nearMiss />)}</div></div> : null}
