...
function PortfolioMatchCard({ match }: { match: PortfolioMatchRecord }) {
  ...
  return <SpCard className="match-card">
    <div className="match-score"><strong>%{match.score}</strong><span>doğrulanmış uyum</span></div>
    <h3>{match.contactName} ↔ {match.portfolioItem.headline}</h3>
    <p>{match.portfolioItem.sourceAuthorName || match.portfolioItem.sharedByName} ile iletişime geçin.</p>
    <small>Kriterlerin %{match.coverage} kadarı karşılaştırılabildi.</small>
    <ul>{match.reasons.filter((reason) => reason.status !== "mismatch").map((reason) => <li className={`match-reason-${reason.status}`} key={reason.key}>{reason.status === "unknown" ? "Doğrulanmalı: " : ""}{reason.detail}</li>)}</ul>
    {copyState === "failed" ? <p className="form-error compact-error">Mesaj kopyalanamadı. Tarayıcı pano iznini kontrol edin.</p> : null}
    <div className="match-card-actions">
      <button className="secondary-action compact-action" onClick={() => void copyMessage()} type="button">{copyState === "copied" ? <Check size={16} /> : <Copy size={16} />}{copyState === "copied" ? "Kopyalandı" : "Mesaj taslağını kopyala"}</button>
...
export function OfficePortfolioSection() {
  ...
  return <section className="office-pool-section" aria-labelledby="office-pool-title">
    <div className="section-heading-row"><div><p className="eyebrow">OFİS ORTAK AĞI</p><h2 id="office-pool-title">Ofis havuzu</h2><p>İlanlı veya ilansız portföyleri paylaş; kayıtlı alıcı talepleriyle açıklanabilir eşleşmeler bul.</p></div><button className="secondary-action inline-action" onClick={() => setOpen(true)} type="button"><Plus size={18} /> Havuza ekle</button></div>
    {matches.length ? <div className="match-strip" aria-label="Uygun eşleşmeler">{matches.slice(0, 6).map((match) => <PortfolioMatchCard key={`${match.contactId}-${match.portfolioItem.id}`} match={match} />)}</div> : null}
    {error && !open ? <p className="form-error notice">{error}</p> : null}{itemsQuery.isPending || matchesQuery.isPending ? <div className="content-state compact"><RefreshCw className="spin" size={20} /> Ofis havuzu taranıyor…</div> : itemsQuery.error || matchesQuery.error ? <p className="form-error notice">{messageFrom(itemsQuery.error ?? matchesQuery.error)}</p> : items.length === 0 ? <SpCard className="office-pool-empty"><Network size={22} /><div><strong>Ortak havuz henüz boş</strong><p>Bir WhatsApp portföy mesajını yapıştırarak ilk kaydı oluşturabilirsiniz.</p></div></SpCard> : <div className="portfolio-pool-grid">{items.slice(0, 8).map((item) => <SpCard className="pool-item-card" key={item.id}><div className="opportunity-top"><span className="stage-badge">{portfolioSourceLabels[item.source]}</span><span>{portfolioAuthorizationLabels[item.authorizationType]}</span></div><h3>{item.headline}</h3><p>{item.location} · {propertyTypeLabels[item.propertyType]}</p><strong>{item.askingPrice ? money(item.askingPrice.amount, item.askingPrice.currency) : "Fiyat belirtilmedi"}</strong><small>{item.sourceAuthorName || item.sharedByName} tarafından paylaşıldı</small><div className="pool-card-actions">{item.listingUrl ? <a className="text-link" href={item.listingUrl} rel="noreferrer" target="_blank">İlanı aç <ExternalLink size={14} /></a> : null}{session && (session.role === "broker" || session.uid === item.ownerUid) ? <button className="text-button danger" disabled={withdrawingId === item.id} onClick={() => void withdraw(item.id)} type="button">{withdrawingId === item.id ? "Kaldırılıyor…" : "Havuzdan kaldır"}</button> : null}</div></SpCard>)}</div>}
