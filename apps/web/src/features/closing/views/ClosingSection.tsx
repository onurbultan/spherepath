...
export function ClosingSection({ listings, showHeading = true }: { listings: ListingRecord[]; showHeading?: boolean }) {
  ...
  {movingDeal ? <Sheet title="İşlem aşaması" close={() => setMovingDeal(null)}><form className="form-stack" onSubmit={advanceDeal}><label>Yeni aşama<select value={dealStage} onChange={(event) => setDealStage(event.target.value as DealStage)}>{nextDealStages(movingDeal.stage).map((item) => <option key={item} value={item}>{dealStageLabels[item]}</option>)}</select></label>{dealStage === "offer" ? <div className="form-row"><label>Teklif<MoneyField currency={currency} value={offerAmount} onChange={setOfferAmount} /></label><label>Para birimi<select value={currency} onChange={(event) => setCurrency(event.target.value as CurrencyCode)}>{(["TRY", "GBP", "USD", "EUR"] as const).map((item) => <option key={item}>{item}</option>)}</select></label></div> : null}{dealStage === "closed" ? <div className="form-stack"><div className="form-row"><label>Gerçekleşen bedel<MoneyField currency={currency} required value={actualAmount} onChange={setActualAmount} /></label><label>Komisyon<MoneyField currency={currency} required value={commissionAmount} onChange={setCommissionAmount} /></label><label>Para birimi<select value={currency} onChange={(event) => setCurrency(event.target.value as CurrencyCode)}>{(["TRY", "GBP", "USD", "EUR"] as const).map((item) => <option key={item}>{item}</option>)}</select></label></div><p className="privacy-hint">Kapama bedeli ve komisyon performans raporlarına kaynak olur.</p></div> : null}{dealStage === "lost" ? <label>Kayıp nedeni<textarea value={lostReason} onChange={(event) => setLostReason(event.target.value)} /></label> : null}{error ? <p className="form-error">{error}</p> : null}<button className="primary-action auth-submit" disabled={pending}>Aşamayı kaydet</button></form></Sheet> : null}</section>;
}
