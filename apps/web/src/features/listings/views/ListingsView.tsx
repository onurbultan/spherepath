"use client";
import { propertyFeaturesForType, portfolioInventorySummary, localDateTimeValue } from "@spherepath/shared";
...
import { ArrowRight, Building2, ChevronDown, Handshake, House, Landmark, MoreHorizontal, Network, Plus, RefreshCw, Store, X } from "lucide-react";
...
import {
  ...
  listingPriceLabel, listingTransitionSchema, nextListingStatuses, opportunitySituation, propertyFeatureLabels, propertyTypeLabels, propertyTypes,
...
export function ListingsView() {
  ...
  const [acquiredAt, setAcquiredAt] = useState(() => localDateTimeValue(nativeNow()));
  ...
  // The pool tab has to name its own size before it is opened, or the number
  // only appears once the advisor is already looking at the list.
  const poolQuery = useQuery({ queryKey: apiQueryKeys.portfolioItems, queryFn: listPortfolioItems });
  ...
  // A primary action's trigger has to be a button: a <summary> reports itself
  // as a generic container, so neither a screen reader nor a test can name it.
  const [addMenuOpen, setAddMenuOpen] = useState(false);
  const addMenuRef = useRef<HTMLDivElement>(null);
  useEffect(() => {
    if (!addMenuOpen) return;
    const close = (event: Event) => { if (!addMenuRef.current?.contains(event.target as Node)) setAddMenuOpen(false); };
    const escape = (event: KeyboardEvent) => { if (event.key === "Escape") setAddMenuOpen(false); };
  ...
  // The pool tab states its own size, and the readiness of the visible set
  // replaces the heading that used to repeat the tab's name back at it.
  const poolItems = poolQuery.data ?? [];
  const readyListingCount = listings.filter((listing) => listingActivationIssues(listing).length === 0).length;
  ...
  async function invalidate() { await Promise.all([queryClient.invalidateQueries({ queryKey: apiQueryKeys.portfolioMatches }), queryClient.invalidateQueries({ queryKey: apiQueryKeys.matchNotifications }), queryClient.invalidateQueries({ queryKey: apiQueryKeys.listings }), queryClient.invalidateQueries({ queryKey: apiQueryKeys.opportunities }), queryClient.invalidateQueries({ queryKey: apiQueryKeys.todayOverview })]); }
  ...
  async function create(event: FormEvent) {
    ...
    const parsed = listingDraftSchema.safeParse({ opportunityId: selectedOpportunityId, address, regionSlug, propertyType, roomCount: propertyType === "land" ? null : roomCount ? Number(roomCount) : null, areaM2: areaM2 ? Number(areaM2) : null, features: features.filter((feature) => propertyFeaturesForType(propertyType).includes(feature)), acquiredAt: new globalThis.Date(acquiredAt).getTime(), authorizationType, askingPrice: parseMoneyInput(askingPrice), currency, expiresAt: null });
  ...
  async function createExisting(event: FormEvent) {
    event.preventDefault(); if (!session) return;
    const parsed = existingListingDraftSchema.safeParse({ ownerContactId, opportunityType: existingOpportunityType, address, regionSlug, propertyType, roomCount: propertyType === "land" ? null : roomCount ? Number(roomCount) : null, areaM2: areaM2 ? Number(areaM2) : null, features: features.filter((feature) => propertyFeaturesForType(propertyType).includes(feature)), acquiredAt: new globalThis.Date(acquiredAt).getTime(), authorizationType, askingPrice: parseMoneyInput(askingPrice), currency, expiresAt: null });
  ...
  return <AppShell><header className="page-header contacts-header"><div><p className="eyebrow">PORTFÖY ENVANTERİ</p><h1>Portföy</h1><p className="context-sentence">{portfolioInventorySummary(listings)}</p><p className="context-sentence">Yetkilerini kaydet ve yayına hazır hâle getir; pazarlama ve kapama İşler ekranında.</p></div><div className="header-actions"><div className="add-listing-menu" ref={addMenuRef}><button aria-expanded={addMenuOpen} aria-haspopup="menu" className="primary-action inline-action" onClick={() => setAddMenuOpen((open) => !open)} type="button"><Plus size={18} /> Portföy ekle<ChevronDown size={15} aria-hidden /></button>{addMenuOpen ? <div role="menu"><button disabled={!contacts.length} onClick={() => { setAddMenuOpen(false); setImportOpen(true); setError(null); }} role="menuitem" type="button"><Building2 size={15} /> <span><strong>Mevcut yetkiyi ekle</strong><small>elindeki portföyü kişiyle ilişkilendir</small></span></button><button onClick={() => { setAddMenuOpen(false); setCreateOpen(true); setError(null); }} role="menuitem" type="button"><Handshake size={15} /> <span><strong>Kazanılan fırsattan ekle</strong><small>yetki alınan işi portföye dönüştür</small></span></button><button onClick={() => { setAddMenuOpen(false); router.replace("/listings?view=pool", { scroll: false }); setPoolOpenSignal((value) => value + 1); }} role="menuitem" type="button"><Network size={15} /> <span><strong>Ofis havuzuna ekle</strong><small>ekiple paylaşılan ilan</small></span></button></div> : null}</div></div></header>
  <div className="section-tabs" role="tablist" aria-label="Portföy görünümü">
    <button aria-selected={portfolioView === "own"} className={portfolioView === "own" ? "active" : ""} onClick={() => router.replace("/listings?view=own", { scroll: false })} role="tab" type="button">Portföyüm <span className="section-tab-count">{listings.length}</span></button>
    <button aria-selected={portfolioView === "pool"} className={portfolioView === "pool" ? "active" : ""} onClick={() => router.replace("/listings?view=pool", { scroll: false })} role="tab" type="button">Ofis havuzu <span className="section-tab-count">{poolItems.length}</span></button>
    <span className="section-tabs-spacer" />
    {portfolioView === "own" && listings.length ? <span className="section-tabs-note">{readyListingCount} portföy yayına hazır{listings.length - readyListingCount ? <> · <strong>{listings.length - readyListingCount} tanesinde eksik var</strong></> : null}</span> : null}
  </div>
  ...
  {listingsQuery.isPending ? <div className="content-state"><RefreshCw className="spin" size={22} /> Portföyler yükleniyor…</div> : listingsQuery.error ? <p className="form-error notice">{messageFrom(listingsQuery.error)}</p> : listings.length === 0 ? <SpCard className="empty-state"><div className="card-icon secondary"><Building2 size={20} /></div><h2>Henüz portföy yok</h2><p>Elindeki mevcut yetkiyi kişiyle ilişkilendirerek doğrudan ekleyebilir veya kazanılan bir fırsatı portföye dönüştürebilirsin.</p><button className="secondary-action" disabled={!contacts.length} onClick={() => setImportOpen(true)} type="button">Mevcut yetkiyi ekle</button></SpCard> : <section className="own-listings-section" aria-label="Kendi portföyün"><div className="listing-table"><div className="listing-table-head"><span>Mülk</span><span>Mülk sahibi</span><span>Yetki</span><span>Liste fiyatı</span><span>Durum</span><span>Yayına hazırlık</span><span /></div>{listings.map((listing) => { const PropertyIcon = listing.propertySummary.type === "villa" || listing.propertySummary.type === "detached_house" ? House : listing.propertySummary.type === "land" ? Landmark : listing.propertySummary.type === "commercial" ? Store : Building2; return <button key={listing.id} onClick={() => openDetail(listing)} type="button"><span className="listing-property"><span className="listing-property-icon"><PropertyIcon size={16} /></span><span><strong>{listing.propertySummary.address}</strong><small>{propertyTypeLabels[listing.propertySummary.type]}{listing.propertySummary.roomCount !== null ? ` · ${listing.propertySummary.roomCount} oda` : ""}{listing.propertySummary.areaM2 !== null ? ` · ${listing.propertySummary.areaM2} m²` : ""}{listing.propertySummary.features.length ? ` · ${listing.propertySummary.features.slice(0, 3).map((item) => propertyFeatureLabels[item]).join(", ")}` : ""}</small></span></span><span>{listing.ownerContactName}</span><span>{authorizationTypeLabels[listing.authorizationType]}</span><strong>{listingPriceLabel(listing.askingPrice, listing.currency)}</strong><span><span className={`stage-badge stage-${listing.status}`}>{listingStatusLabels[listing.status]}</span></span><span className="listing-readiness">{(() => { const items = listingReadiness(listing); const ready = items.filter((item) => item.ready).length; const missing = listingActivationIssues(listing); const complete = ready === items.length; return <><span className="listing-readiness-meter"><span className={complete ? "is-ready" : "is-partial"} style={{ width: `${Math.round((ready / items.length) * 100)}%` }} /></span><small className={complete ? "" : "is-partial-text"} title={complete ? undefined : `${missing.join(", ")} eksik`}>{complete ? `Yayında · ${Math.max(0, Math.floor((Date.now() - listing.acquiredAt) / 86_400_000))} gün` : `${ready}/${items.length} · ${missing.length} eksik`}</small></>; })()}</span><span className="listing-more" aria-hidden><MoreHorizontal size={17} /></span></button>; })}</div></section>}
  {listings.length ? <Link className="listing-work-link" href="/opportunities#closing"><Handshake size={16} aria-hidden /><span>Bu portföylerdeki sunum, gezi ve teklif takibi <strong>İşler</strong> ekranında</span><ArrowRight size={15} aria-hidden /></Link> : null}
  </> : null}
