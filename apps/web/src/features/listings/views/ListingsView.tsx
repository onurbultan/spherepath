...
import {
  ...
  listingPriceLabel, listingTransitionSchema, nextListingStatuses, propertyFeatureLabels, propertyFeatures, propertyTypeLabels, propertyTypes,
  type AuthorizationType, type CurrencyCode, type ListingStatus, type OpportunityType, type PropertyFeature, type PropertyType } from "@spherepath/shared";
...
export function ListingsView() {
  ...
  const totalTryValue = listings.filter((listing) => listing.currency === "TRY" && listing.status !== "removed").reduce((sum, listing) => sum + (listing.askingPrice ?? 0), 0);
