...
export function ListingsView() {
  ...
  return <AppShell><header className="page-header contacts-header"><div><p className="eyebrow">AKTİF ENVANTER</p><h1>Portföy</h1><p className="context-sentence">Kazanılan fırsatı mülk ve yetki kaydına dönüştür, yaşam döngüsünü ve kapama akışını takip et.</p></div><button className="primary-action inline-action" disabled={!candidates.length} onClick={() => { setCreateOpen(true); setError(null); }} type="button"><Plus size={18} /> Portföy ekle</button></header>
  <OfficePortfolioSection />
  {error && !createOpen && !moving ? <p className="form-error notice">{error}</p> : null}
  {listingsQuery.isPending ? <div className="content-state"><RefreshCw className="spin" size={22} /> Portföyler yükleniyor…</div> : listingsQuery.error ? <p className="form-error notice">{messageFrom(listingsQuery.error)}</p> : listings.length === 0 ? <SpCard className="empty-state"><div className="card-icon secondary"><Building2 size={20} /></div><h2>Henüz portföy yok</h2><p>Bir satıcı veya kiraya veren fırsatını “Kazanıldı” aşamasına getirerek portföye dönüştür.</p></SpCard> : <section className="own-listings-section" aria-labelledby="own-listings-title"><div className="inline-section-heading"><div><h2 id="own-listings-title">Kendi portföyün</h2><span>yetkiye bağlanmış, sende kayıtlı mülkler</span></div><span className="listing-total">Toplam liste değeri <strong>{money(totalTryValue, "TRY")}</strong></span></div><div className="listing-table"><div className="listing-table-head"><span>Mülk</span><span>Mülk sahibi</span><span>Yetki</span><span>Liste fiyatı</span><span>Durum</span><span>Yayında</span></div>{listings.map((listing) => <button key={listing.id} onClick={() => { if (nextListingStatuses(listing.status).length) openMove(listing); }} type="button"><span className="listing-property"><strong>{listing.propertySummary.address}</strong><small>{propertyTypeLabels[listing.propertySummary.type]}{listing.propertySummary.roomCount !== null ? ` · ${listing.propertySummary.roomCount} oda` : ""}{listing.propertySummary.areaM2 !== null ? ` · ${listing.propertySummary.areaM2} m²` : ""}{listing.propertySummary.features.length ? ` · ${listing.propertySummary.features.slice(0, 2).map((item) => propertyFeatureLabels[item]).join(", ")}` : ""}</small></span><span>{listing.ownerContactName}</span><span>{authorizationTypeLabels[listing.authorizationType]}</span><strong>{money(listing.askingPrice, listing.currency)}</strong><span><span className={`stage-badge stage-${listing.status}`}>{listingStatusLabels[listing.status]}</span></span><span>{listing.status === "preparing" ? "—" : `${Math.max(0, Math.floor((Date.now() - listing.acquiredAt) / 86_400_000))} gün`}</span></button>)}</div></section>}
