...
import { apiQueryKeys, contactSourceLabels, reportingPeriodLabels, reportingPeriods, type CurrencyCode, type ReportingPeriod } from "@spherepath/shared";
  ...
  return <AppShell><div className="funnel-view">
    ...
    {query.isPending ? <p className="context-sentence">Huni hazırlanıyor…</p> : query.error ? <p className="form-error notice">Huni yüklenemedi.</p> : <div className="funnel-workspace">
      <section className="sp-card earnings-card" aria-label="Dönem kazancı">
        ...
        {earnings && earnings.totals.length ? <div className="earnings-totals">{earnings.totals.map((total) => <div className="earnings-total" key={total.currency}><strong>{money(total.commission, total.currency)}</strong><small>{total.closedCount} kapanan işlem · {money(total.volume, total.currency)} hacim{total.commissionRate !== null ? ` · %${(total.commissionRate * 100).toFixed(1)} komisyon` : ""}</small></div>)}</div> : <p className="context-sentence">Bu dönemde kapanan işlem yok. Bir işlemi kapattığında komisyonun burada toplanır.</p>}
      ...
      <section className="sp-card mirror-card" aria-label="Kendi aynan">
        ...
        {!metrics ? null : !metrics.sampleSufficient ? <p className="context-sentence">Henüz güvenilir bir sonuç çıkaracak kadar kayıt yok. Birkaç görüşme daha kaydettiğinde buradaki oranlar anlamlı olmaya başlar.</p> : <>
          <div className="mirror-tiles">
            ...
            {metrics.stageDurations[0] ? <div className="mirror-tile"><strong>{metrics.stageDurations[0].medianDays} gün</strong><small>En yavaş aşama<br />{metrics.stageDurations[0].stage}</small></div> : null}
