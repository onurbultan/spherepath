  ...
  return <AppShell><div className="funnel-view">
    ...
    {query.isFetching && !query.isPending ? <p role="status" className="context-sentence">Seçilen dönem hesaplanıyor; önceki dönem gösteriliyor…</p> : null}{query.isPending ? <p className="context-sentence" role="status">Kişiler, fırsatlar ve kapanışlar hesaplanıyor…</p> : query.error ? <p className="form-error notice">Huni yüklenemedi.</p> : <div className="funnel-workspace">
      <section className="sp-card earnings-card" aria-label="Dönem kazancı">
        <div className="earnings-heading"><div><p className="eyebrow">{reportingPeriodLabels[period].toLocaleUpperCase("tr-TR")} İÇİNDE</p><h2>{earnings && !earnings.totals.length && earnings.pipeline.length ? "Boru hattındaki değer" : "Kazancın"}</h2></div>{target?.periodTarget !== null && target !== undefined ? <span className="earnings-target"><small>Portföy hedefi</small><strong>{target.achieved} / {target.periodTarget}</strong></span> : null}</div>
        {earnings && earnings.totals.length ? <div className="earnings-totals">{earnings.totals.map((total) => <div className="earnings-total" key={total.currency}><strong>{money(total.commission, total.currency)}</strong><small>{total.closedCount} kapanan işlem · {money(total.volume, total.currency)} hacim{total.commissionRate !== null ? ` · %${(total.commissionRate * 100).toFixed(1)} komisyon` : ""}</small></div>)}</div>
          : earnings && earnings.pipeline.length ? <div className="earnings-totals">{earnings.pipeline.map((entry) => <div className="earnings-total is-pipeline" key={entry.currency}><strong>{money(entry.amount, entry.currency)}</strong><small>{entry.count} işlem teklif veya sözleşme aşamasında. Bu dönemde kapanan işlem yok, bu yüzden kazanç değil potansiyel.</small></div>)}</div>
      ...
      <section className="sp-card funnel-panel" aria-label="Satış hunisi"><div className="funnel-panel-heading"><div><p className="eyebrow">{reportingPeriodLabels[period].toLocaleUpperCase("tr-TR")}</p><h2>Beş adımda durumun</h2></div><details className="funnel-ratio-note"><summary>Oranlar nasıl hesaplanıyor?</summary><p>{funnelCountRatioCopy}</p></details></div><div className="funnel-stage-list">{stages.map((stage, index) => {
      ...
      <section className="sp-card mirror-card" aria-label="Kendi aynan">
        ...
        {!metrics ? null : !metrics.sampleSufficient ? <p className="context-sentence">Henüz güvenilir bir sonuç çıkaracak kadar kayıt yok. Birkaç görüşme daha kaydettiğinde buradaki oranlar anlamlı olmaya başlar.</p> : <>
          <div className="mirror-tiles">
            ...
            {metrics.timeToWonDays !== null && metrics.timeToWonDays > 0 ? <div className="mirror-tile"><strong>{metrics.timeToWonDays} gün</strong><small>Kazanmaya kadar<br />geçen süre</small></div> : null}
            {metrics.stageDurations[0] && metrics.stageDurations[0].medianDays > 0 ? <div className="mirror-tile"><strong>{metrics.stageDurations[0].medianDays} gün</strong><small>En yavaş aşama<br />{stageName(metrics.stageDurations[0].stage)}</small></div> : null}
          ...
          {metrics.timeToWonDays === null || metrics.timeToWonDays === 0 ? <p className="context-sentence funnel-pending-metric">Kazanma hızı ve en yavaş aşama, ilk işlem kapandığında hesaplanacak.</p> : null}
        </>}
      </section>
    </div>}
  </div></AppShell>;
