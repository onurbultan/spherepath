...
export function OpportunitiesView() {
  ...
  return <AppShell>
    <header className="page-header contacts-header"><div><p className="eyebrow">PORTFÖY ÜRETİMİ</p><h1>Fırsatlar</h1><p className="context-sentence">Her lead’i net bir aşama ve kabul edilmiş sonraki aksiyonla ilerlet. Aşama geçişleri sunucuda doğrulanır.</p></div><div className="header-actions"><div className="segmented-control"><button className={view === "board" ? "selected" : ""} onClick={() => setView("board")} type="button"><LayoutGrid size={13} /> Pano</button><button className={view === "list" ? "selected" : ""} onClick={() => setView("list")} type="button"><List size={13} /> Liste</button></div><button className="secondary-action inline-action" disabled={!openOpportunities.length} onClick={() => openOpportunities[0] && openMove(openOpportunities[0])} type="button">Sıradakini ilerlet <ArrowRight size={15} /></button><button className="primary-action inline-action" disabled={!contacts.length} onClick={() => { setCreateOpen(true); setError(null); }} type="button"><Plus size={18} /> Yeni fırsat</button></div></header>
