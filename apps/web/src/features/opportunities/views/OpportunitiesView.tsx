...
export function OpportunitiesView() {
  ...
  async function create(event: FormEvent<HTMLFormElement>) {
    ...
    try { await saveOpportunity(session, parsed.data); closeCreate(); await invalidate(); }
  ...
  function openCorrection(opportunity: OpportunityRecord) {
    setCorrecting(opportunity); setTargetStage(opportunity.stage === "new_lead" ? "first_contact" : "new_lead"); setActionType("call"); setActionAt(localDateTime()); setCorrectionReason(""); setLostReason(""); setError(null);
  }

  async function correct(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (!session || !correcting) return;
    const terminal = targetStage === "won" || targetStage === "lost";
    const parsed = opportunityStageCorrectionSchema.safeParse({ opportunityId: correcting.id, toStage: targetStage, reason: correctionReason, lostReason: targetStage === "lost" ? lostReason.trim() || null : null, nextActionType: terminal ? null : actionType, nextActionAt: terminal ? null : new Date(actionAt).getTime() });
    if (!parsed.success) return setError(parsed.error.issues[0]?.message ?? "Düzeltme bilgilerini kontrol et.");
    setPending(true); setError(null);
    try { await correctOpportunity(session, parsed.data); setCorrecting(null); await invalidate(); }
    catch (nextError) { setError(messageFrom(nextError)); }
    finally { setPending(false); }
  }

  return <AppShell>
    <header className="page-header contacts-header"><div><p className="eyebrow">AKTİF TALEPLER</p><h1>Fırsatlar</h1><p className="context-sentence">Her talebi bulunduğu aşama ve sıradaki işle birlikte ilerlet.</p></div><div className="header-actions"><div className="segmented-control"><button className={view === "board" ? "selected" : ""} onClick={() => setView("board")} type="button"><LayoutGrid size={13} /> Pano</button><button className={view === "list" ? "selected" : ""} onClick={() => setView("list")} type="button"><List size={13} /> Liste</button></div><button className="secondary-action inline-action" disabled={!nextOpportunity} onClick={() => nextOpportunity && openMove(nextOpportunity)} type="button">{nextOpportunity ? `${nextOpportunity.subjectContactName}: ilerlet` : "İlerletilecek fırsat yok"} <ArrowRight size={15} /></button><button className="primary-action inline-action" disabled={!contacts.length} onClick={() => { setCreateOpen(true); setError(null); }} type="button"><Plus size={18} /> Yeni fırsat</button></div></header>
    {error && !activeCreateOpen && !moving ? <p className="form-error notice">{error}</p> : null}
    {opportunities.length ? <><div className="opportunity-insights"><button aria-pressed={actionFilter === "missing"} className={missingActionCount ? "warning" : ""} onClick={() => setActionFilter((current) => current === "missing" ? "all" : "missing")} type="button">{missingActionCount} fırsatta sonraki aksiyon yok</button><button aria-pressed={actionFilter === "overdue"} className={overdueCount ? "danger" : ""} onClick={() => setActionFilter((current) => current === "overdue" ? "all" : "overdue")} type="button">{overdueCount} aksiyon gecikti</button><span>Aşamada ortalama <strong>{averageStageDays.toLocaleString("tr-TR", { maximumFractionDigits: 1 })} gün</strong></span><span className="opportunity-outcomes">Kazanıldı · {opportunities.filter((item) => item.stage === "won").length}</span><span>Kaybedildi · {opportunities.filter((item) => item.stage === "lost").length}</span></div><div className="opportunity-filterbar"><label className="contact-search"><Search size={16} aria-hidden /><input aria-label="Fırsatlarda ara" placeholder="Kişi, bölge veya talep ara" type="search" value={search} onChange={(event) => setSearch(event.target.value)} /></label><label><span className="sr-only">Fırsat türü</span><select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value as OpportunityType | "all")}><option value="all">Tüm fırsat türleri</option>{opportunityTypes.map((item) => <option key={item} value={item}>{opportunityTypeLabels[item]}</option>)}</select></label><strong>{openOpportunities.length} açık fırsat</strong></div></> : null}
    {opportunitiesQuery.isPending ? <div className="content-state"><RefreshCw className="spin" size={22} /> Fırsatlar yükleniyor…</div> : opportunitiesQuery.error ? <p className="form-error notice">{messageFrom(opportunitiesQuery.error)}</p> : opportunities.length === 0 ? <SpCard className="empty-state"><div className="card-icon secondary"><BriefcaseBusiness size={20} /></div><h2>İlk fırsatını oluştur</h2><p>Kayıtlı bir kişiyi talebe dönüştür ve sıradaki gerçek işi belirle.</p>{contacts.length ? <button className="secondary-action" onClick={() => setCreateOpen(true)} type="button">Fırsat oluştur</button> : <p>Önce bir kişi eklemelisin.</p>}</SpCard> : openOpportunities.length === 0 ? <SpCard className="empty-state"><div className="card-icon secondary"><Search size={20} /></div><h2>Eşleşen açık fırsat yok</h2><p>Arama metnini veya filtreleri değiştir.</p><button className="secondary-action" onClick={() => { setSearch(""); setTypeFilter("all"); setActionFilter("all"); }} type="button">Filtreleri temizle</button></SpCard> : view === "board" ? <section className="opportunity-board" aria-label="Fırsat panosu">{activeStages.map((stage) => {
