...
export function NoteProcessingSheet({ item, contacts, initialKind, onClose, onChanged }: { item: InboxItemRecord; contacts: readonly ContactRecord[]; initialKind?: InboxItemKind; onClose(): void; onChanged(updatedItem?: InboxItemRecord): Promise<void> | void }) {
  ...
  const [personName, setPersonName] = useState(item.analysis?.insights.contactName?.trim() || inferred.explicitContact?.fullName || "");
  const [personPhone, setPersonPhone] = useState(item.analysis?.insights.contactPhone?.trim() || inferred.explicitContact?.phone || "");
    ...
    try {
      ...
      if (kind === "person") {
        ...
        processedItem = (await processItem(session, {
          ...
          action: "person",
          contact,
          approvedInsights: analysis?.insights ?? item.analysis?.insights,
          recordInteraction: true,
          opportunityType: createPersonOpportunity ? opportunityType : null,
        })).item;
        ...
        processedItem = (await processItem(session, { inboxItemId: item.id, action: "requirement", contactId, opportunityType: requirementOpportunity(opportunityType), nextActionType: actionType, nextActionAt: new Date(actionAt).getTime(), approvedInsights: analysis?.insights ?? emptyVoiceInsights })).item;
